...
file_path = r'C:\Users\Harry Murphy\OneDrive\Desktop\Harsh_Semi_adjusted.csv'
...
smoothed_dayrate_extended = processed_data['Max Lead Dayrate'].rolling(window=6, center=True).mean().fillna(method='ffill').fillna(method='bfill')
...
valid_end_date = validation_data.index.max()  # Ensure the end date matches a valid index date
validation_predictions = sarimax_results_final.get_prediction(start=validation_period, end=valid_end_date, exog=validation_exog).predicted_mean
